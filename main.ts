...
// BankAccount interface definition
...
console.log(
  chalk.bold.hex("#ffd700")(`\t   Wlecome To Hiba M.Dawood Bank-System`)
);
console.log(chalk.bold.hex("#7cfc00")("=".repeat(60)));
  ...
  do { 
      ...
      if (service.select == "View Balance") {
        ...
        if (account) {
          ...
          console.log(
            chalk.hex("#ffb6c1")(
              `Dear ${chalk.bold.italic.greenBright(name?.fullName)} ${
                name?.lastName
              } your account blanace is ${chalk.bold.italic.yellow(
                `$${account.balance}`
              )}\n`
            )
          );
      ...
      if (service.select == "Cash Deposit") {
        ...
        if (account) {
          let ans = await inquirer.prompt({
            type: "number",
            message: chalk.bold.italic.hex("#90ee90")(
